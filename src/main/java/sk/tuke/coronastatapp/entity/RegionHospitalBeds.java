...
public class RegionHospitalBeds {
    ...
    @ManyToOne
    @JoinColumn(name = "Region.id", nullable = true) //Interné id regiónu z regiónov z /api/regions alebo null. Hodnota null znamená, že dáta nie sú priradené žiadnemu kraju.
    ...
    public RegionHospitalBeds(Date oldestReportedAt, Date newestReportedAt, Date publishedOn,
                              Region region, int id, int capacityAll, int freeAll, int capacityCovid,
                              int occupiedJisCovid, int occupiedOaimCovid, int occupiedO2Covid,
                              int occupiedOtherCovid, Date updatedAt) {
